document.addEventListener("DOMContentLoaded", () => {
  ...
  [homeLink, personalLink, archiveLink].forEach(l => l.onclick = () => { menu.style.right="-260px"; menuOpen=false; });

  // ZITATE LADEN
  fetch("quotes.json").then(r=>r.ok?r.json():Promise.reject("quotes.json nicht gefunden"))
  ...
  function showDailyQuote(){
    ...
    const idx=getDayOfYear()%quotesData.daily.length;
    const text=quotesData.daily[idx];
  ...
  // ARCHIV
  const months=[];
  for(let y=2025;y<=2026;y++){ const startM=y===2025?12:1; for(let m=startM;m<=12;m++){ months.push({year:y,month:m}); } }

  function showMonths(){
    monthDetail.style.display="none"; backBtn.style.display="none"; monthsContainer.style.display="flex"; monthsContainer.innerHTML="";
  ...
  function showMonthDetail(year,month){
    monthsContainer.style.display="none"; backBtn.style.display="block"; monthDetail.style.display="block"; monthDetail.innerHTML="";
    ...
    if(monthData.length===0){ const p=document.createElement("p"); p.innerText="Keine Zitate für diesen Monat."; monthDetail.appendChild(p); }
    else{ monthData.forEach(e=>{ const div=document.createElement("div"); div.className="archiveItem"; div.innerHTML=`<div class="date">${e.date}</div>${e.text}`; monthDetail.appendChild(div); }); }
  }
  ...
  function showPersonalMenu(){
    personalDetail.style.display="none"; personalBackBtn.style.display="none"; personalMenu.style.display="flex"; personalMenu.innerHTML="";
    personalSections.forEach(s=>{
      const btn=document.createElement("button"); btn.innerText=s.title; btn.onclick=()=>showPersonalDetail(s.id); personalMenu.appendChild(btn);
    });
  }

  function showPersonalDetail(id){
    personalMenu.style.display="none"; personalBackBtn.style.display="block"; personalDetail.style.display="block"; personalDetail.innerHTML="";
    const h3=document.createElement("h3"); h3.innerText=personalSections.find(s=>s.id===id).title; personalDetail.appendChild(h3);

    if(id==="notes"){
      const textarea=document.createElement("textarea"); textarea.style.minHeight="150px"; textarea.value=localStorage.getItem("personalNotes")||""; personalDetail.appendChild(textarea);
      const saveBtn=document.createElement("button"); saveBtn.innerText="Notizen speichern"; saveBtn.style.marginTop="10px";
      saveBtn.onclick=()=>{ localStorage.setItem("personalNotes",textarea.value); alert("Notizen gespeichert!"); };
      personalDetail.appendChild(saveBtn);
    }

    if(id==="weeklyQuote"){
      const input=document.createElement("input"); input.type="text"; input.value=localStorage.getItem("personalWeeklyQuote")||""; personalDetail.appendChild(input);
      const saveBtn=document.createElement("button"); saveBtn.innerText="Zitat speichern"; saveBtn.style.marginTop="10px";
      saveBtn.onclick=()=>{ localStorage.setItem("personalWeeklyQuote",input.value); alert("Zitat gespeichert!"); };
      personalDetail.appendChild(saveBtn);
    }
  }
  ...
  // START
  updateHeader(); updateButtons(); updateYearCountdown(); showDailyQuote();
